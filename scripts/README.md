...
Scripts are grouped by purpose — `bringup/` puts the stack/robot into a
desired state, `evaluation/` measures it, `fault_injection/` corrupts sensor
streams to prove robustness. The install destination is flat, so every tool
is invoked the same way regardless of folder: `ros2 run olive <script>`.
...
| `analyze_bag.py <bag>` | Offline counterpart to `ate_eval.py`: reads a recorded bag (mcap/sqlite3, zstd file-compression handled automatically), prints each odometry stream's frame_id + first position, the fused-vs-ground-truth error split at the marker anchor with an early-vs-late drift check, the smooth-stream continuity report (`--local-topic`, `--max-step` — the local odom must never jump while map->odom absorbs the corrections), and a monocular-VO health line (`--vo-topic`: update count, path-length ratio vs ground truth, start-aligned drift). |
| `plot_results.py` | Regenerates the README result figures (`media/*.png`) from a recorded bag: trajectory-vs-ground-truth, smooth-local-vs-raw-wheel in the odom frame, error-over-time, the REP-105 continuity split, the online IMU bias/velocity, and (when `/olive/visual_odom` is present) the start-aligned monocular-VO trajectory. Point it at a bag with `OLIVE_BAG=<path> ros2 run olive plot_results.py`. |
...
| `endurance_test.py [--minutes]` | Long-run soak: drives repeating patterns while sampling fusion_node RSS, reporting whether memory plateaus (bounded keyframe-cloud storage working) and the estimate stays sane. |
